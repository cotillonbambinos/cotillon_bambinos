...
(function () {
  ...
  function waMsgLink(items, customerName) {
    ...
    items.forEach(function (item) {
      msg += '• ' + item.qty + ' x ' + cartItemLabel(item) + ' — $' + (item.price * item.qty).toLocaleString('es-AR') + '\n';
    });
  ...
  function cartLoad() {
    try {
      ...
      /* Compatibilidad con carritos guardados antes del sistema de variantes */
      items.forEach(function (i) { if (!i.key) i.key = String(i.id); });
      return items;
    } catch (e) { return []; }
  }

  /* Nombre a mostrar en el carrito y en el mensaje de WhatsApp: incluye la variante elegida */
  ...
  function cartAdd(productId, variantId, qty) {
    ...
    /* Si el producto tiene variantes, se agrega la elegida (o la primera por defecto) */
    var variant = null;
    if (prod.variants && prod.variants.length) {
      variant = prod.variants.find(function (v) { return v.id === variantId; }) || prod.variants[0];
    }

    var key = variant ? (productId + '::' + variant.id) : String(productId);
    var price = variant ? variant.price : prod.price;
    var img = (variant && variant.img) ? variant.img : prod.img;
  ...
  /* Trampa de foco genérica: mientras un contenedor modal está abierto,
     Tab/Shift+Tab ciclan solo entre sus elementos enfocables. Se reutiliza
     tanto para el carrito como para el modal de Vista Rápida. */
  function trapFocusIn(container, e) {
    if (e.key !== 'Tab' || !container) return;
    var focusables = els('button, a[href], input, select, [tabindex]:not([tabindex="-1"])', container)
  ...
  function cartRender() {
    ...
    body.innerHTML = cart.map(function (item) {
      return '<div class="cart-item" data-key="' + item.key + '">' +
        ...
        '<div class="cart-item-name">' + cartItemLabel(item) + '</div>' +
        ...
        '<button class="cart-item-del" data-key="' + item.key + '" aria-label="Eliminar">×</button>' +
        '</div>' +
        '</div>';
    }).join('');
    ...
    els('.qty-btn.minus', body).forEach(function (btn) {
      btn.addEventListener('click', function () { cartChange(btn.dataset.key, -1); });
    });
    els('.qty-btn.plus', body).forEach(function (btn) {
      btn.addEventListener('click', function () { cartChange(btn.dataset.key, 1); });
    });
    els('.cart-item-del', body).forEach(function (btn) {
      btn.addEventListener('click', function () { cartRemove(btn.dataset.key); });
    });
  }
  ...
  function productCard(prod) {
    ...
    return '<div class="prod-card" data-id="' + prod.id + '" data-cat="' + prod.category + '" data-price="' + prod.price + '" data-rating="' + prod.rating + '">' +
      ...
      '<button class="prod-name prod-name-btn" type="button">' + prod.name + '</button>' +
  ...
  function bindAddToCart(ctx) {
    els('.btn-add-cart:not([disabled])', ctx).forEach(function (btn) {
      btn.addEventListener('click', function (e) {
        ...
        var prod = D.products.find(function (p) { return p.id === id; });
        var variantId = (prod && prod.variants && prod.variants.length) ? prod.variants[0].id : undefined;
        cartAdd(id, variantId, 1);
  ...
  /* ── bind quick view (abre el modal al hacer clic en la card,
     excepto en el botón "Agregar al carrito") ─────────────── */
  function bindQuickView(ctx) {
    els('.prod-card', ctx).forEach(function (card) {
      card.addEventListener('click', function (e) {
        if (e.target.closest('.btn-add-cart')) return;
        qvOpen(+card.dataset.id);
      });
    });
    els('.prod-name-btn', ctx).forEach(function (btn) {
      btn.addEventListener('click', function (e) {
        e.stopPropagation();
        var card = btn.closest('.prod-card');
        if (card) qvOpen(+card.dataset.id);
      });
    });
  }
  ...
  /* ── QUICK VIEW: modal único y reutilizable ──────────────── */
  var qvState = { product: null, variant: null, qty: 1 };
  var qvLastFocusedEl = null;

  function qvIsOpen() {
    var modal = el('#qv-modal');
    return !!(modal && modal.classList.contains('open'));
  }

  /* Oculta el modal sin mover el foco (se usa al agregar al carrito,
     donde el foco debe pasar al carrito, no volver atrás). */
  function qvHide() {
    var modal = el('#qv-modal');
    var overlay = el('#qv-overlay');
  ...
  function qvClose() {
    qvHide();
    if (qvLastFocusedEl && typeof qvLastFocusedEl.focus === 'function') qvLastFocusedEl.focus();
  }

  function qvOpen(productId) {
    var prod = D.products.find(function (p) { return p.id === productId; });
    if (!prod) return;

    qvState.product = prod;
    qvState.variant = (prod.variants && prod.variants.length) ? prod.variants[0] : null;
    qvState.qty = 1;
    qvRender();

    var modal = el('#qv-modal');
    var overlay = el('#qv-overlay');
    qvLastFocusedEl = document.activeElement;
    ...
    var closeBtn = el('#qv-close');
    if (closeBtn) closeBtn.focus();
  }

  function qvSelectVariant(variantId) {
    var prod = qvState.product;
    if (!prod || !prod.variants) return;
    var variant = prod.variants.find(function (v) { return v.id === variantId; });
    if (!variant) return;
  ...
  function qvChangeQty(delta) {
    qvState.qty = Math.max(1, Math.min(99, qvState.qty + delta));
    var val = el('#qv-qty-val');
    if (val) val.textContent = qvState.qty;
  }

  /* Vuelve a pintar todo el contenido del modal según el producto
     y la variante actualmente seleccionados (imagen, precio, stock, etc). */
  function qvRender() {
    var prod = qvState.product;
    if (!prod) return;
    var variant = qvState.variant;

    var price = variant ? variant.price : prod.price;
    var img = (variant && variant.img) ? variant.img : prod.img;
    var stock = variant ? variant.stock : prod.stock;
    var desc = (variant && variant.desc) ? variant.desc : (prod.desc || '');

    var imgEl = el('#qv-img');
    if (imgEl) {
      imgEl.src = img || 'assets/img/placeholder.svg';
      imgEl.alt = prod.name + (variant ? ' - ' + variant.label : '');
      imgEl.onerror = function () { imgEl.src = 'assets/img/placeholder.svg'; };
    }

    var catEl = el('#qv-cat');
    if (catEl) catEl.textContent = prod.category + (prod.medida ? ' · ' + prod.medida : '');

    var nameEl = el('#qv-name');
    if (nameEl) nameEl.textContent = prod.name;

    var starsEl = el('#qv-stars');
    if (starsEl) starsEl.innerHTML = stars(prod.rating) + ' <span class="prod-rating-num">(' + prod.rating + ')</span>';

    var priceEl = el('#qv-price');
    if (priceEl) priceEl.textContent = formatPrice(price);

    var stockEl = el('#qv-stock');
    if (stockEl) {
      var stockClass = stock === 'agotado' ? 'stock-out' : stock === 'poco stock' ? 'stock-low' : 'stock-ok';
      var stockLabel = stock === 'agotado' ? 'Sin stock' : stock === 'poco stock' ? '⚡ Poco stock' : '✓ Disponible';
      stockEl.className = 'prod-stock ' + stockClass;
      stockEl.textContent = stockLabel;
    }

    var descEl = el('#qv-desc');
    if (descEl) descEl.textContent = desc;

    /* chips de variantes */
    var groupEl = el('#qv-variant-group');
    var labelEl = el('#qv-variant-label');
    var chipsEl = el('#qv-chips');
    if (prod.variants && prod.variants.length && groupEl && chipsEl) {
      groupEl.classList.remove('hidden');
      if (labelEl) labelEl.textContent = prod.variantLabel || 'Opciones';
      chipsEl.innerHTML = prod.variants.map(function (v) {
        var isActive = variant && v.id === variant.id;
        var isOut = v.stock === 'agotado';
        var swatch = v.swatch ? '<span class="qv-chip-swatch" style="background:' + v.swatch + '"></span>' : '';
        return '<button type="button" class="qv-chip' + (isActive ? ' active' : '') + (isOut ? ' out-of-stock' : '') +
          '" data-variant-id="' + v.id + '"' + (isOut ? ' disabled' : '') + ' aria-pressed="' + (isActive ? 'true' : 'false') + '">' +
          swatch + v.label + '</button>';
      }).join('');
      els('.qv-chip', chipsEl).forEach(function (chip) {
        chip.addEventListener('click', function () { qvSelectVariant(chip.dataset.variantId); });
      });
    } else if (groupEl) {
    ...
    /* cantidad */
    var qtyVal = el('#qv-qty-val');
    if (qtyVal) qtyVal.textContent = qvState.qty;

    /* botón agregar */
    var addBtn = el('#qv-add-btn');
    if (addBtn) {
      if (stock === 'agotado') {
  ...
  function initQuickView() {
    var overlay = el('#qv-overlay');
    var closeBtn = el('#qv-close');
    var minusBtn = el('#qv-qty-minus');
    var plusBtn = el('#qv-qty-plus');
    var addBtn = el('#qv-add-btn');

    if (overlay) overlay.addEventListener('click', qvClose);
    if (closeBtn) closeBtn.addEventListener('click', qvClose);
    if (minusBtn) minusBtn.addEventListener('click', function () { qvChangeQty(-1); });
    if (plusBtn) plusBtn.addEventListener('click', function () { qvChangeQty(1); });

    if (addBtn) {
      addBtn.addEventListener('click', function () {
        if (!qvState.product) return;
        var variantId = qvState.variant ? qvState.variant.id : undefined;
        var qty = qvState.qty;
        var productId = qvState.product.id;
        qvHide(); /* el foco pasa al carrito, que se abre a continuación */
  ...
  function initA11y() {
    document.addEventListener('keydown', function (e) {
      ...
      var qvModal = el('#qv-modal');
      if (qvModal && qvModal.classList.contains('open')) {
        trapFocusIn(qvModal, e);
        return;
      }
      var cartDrawer = el('#cart-drawer');
      if (cartDrawer && cartDrawer.classList.contains('open')) {
        trapFocusIn(cartDrawer, e);
      }
    });
  }
