...
(function () {
  ...
  function initCatalog() {
    ...
    function renderCatalog() {
      ...
      if (prods.length === 0) {
        grid.innerHTML = '<div class="catalog-empty"><div class="catalog-empty-icon">🔍</div><p>No encontramos productos con esos filtros</p><button class="btn-reset-filters" onclick="document.querySelector(\'#catalog-search\').value=\'\';document.querySelector(\'.filter-btn[data-cat=Todos]\').click()">Ver todos</button></div>';
        return;
      }
  ...
  /* ── MAIN BOOT ───────────────────────────────────────────── */
  function boot() {
    if (!D) { console.error('[Bambinos] manifest.js no cargó'); return; }
